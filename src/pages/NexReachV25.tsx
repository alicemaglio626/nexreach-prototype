            ...
            <Tabs variant="pill" defaultValue="workspace">
              ...
              <Tabs.Panel value="workspace">
                <Stack gap="xl">
                  ...
                  {/* Global Outcomes */}
                  <Box>
                    <Text fw={600} size="md" mb={4}>Global Outcomes</Text>
                              ...
                              <Box style={{ padding: '12px 16px' }}>
                                ...
                                <Group gap={20}>
                                  {['Scheduled', 'Progress Logged', ...(retrievalMethod === 'onsite' ? ['No Availability'] : []), 'In Research', 'Rerouted', 'Pended'].map(s => (
                                    <Box key={s}>
                                      <Text size="xs" style={{ color: '#6e6d6a' }}>{s === 'Progress Logged' ? 'Progress Saved' : s === 'In Research' ? 'Sent to Research' : s}</Text>
                                      <Text size="md" fw={700} style={{ color: '#242423' }}>{statusCounts[s] || 0}</Text>
                                    </Box>
                                  ))}
                                </Group>
                              </Box>
                            ...
                            <Box style={{ borderTop: '1px solid #e7e5df', padding: '12px 16px', backgroundColor: '#fff' }}>
                              ...
                              <Group gap={20}>
                                {(isInbound
                                  ...
                                  : ['Outreach In Progress', 'In Research', 'Rerouted', 'Pended']
                                ).map(s => (
                                  <Box key={s}>
                                    <Text size="xs" style={{ color: '#6e6d6a' }}>{s === 'Progress Logged' ? 'Progress Saved' : s === 'In Research' ? 'Sent to Research' : s}</Text>
                                    <Text size="md" fw={700} style={{ color: '#242423' }}>{statusCounts[s] || 0}</Text>
      ...
      {activeAction === 'schedule' && (
        isEmrRemote || (isInbound && effectiveMethod === 'emr-remote')
          ...
          : (retrievalMethod === 'onsite' || (isInbound && effectiveMethod === 'onsite'))
            ...
            : <ScheduleModal count={actionScope === 'global' ? requestRows.length : selectedRows.size} onClose={() => { setActiveAction(null); setActionScope('selected'); }} onSubmit={(date, pmtReq) => applyAction('schedule', actionScope === 'global', undefined, date, pmtReq)} />
      )}
