...
import {
  ...
  LastIcon,
  ChevronLeftIcon,
  ChevronRight as DartChevronRight,
} from '@datavant/dart';
import {
  IconSearch,
  IconRefresh,
...
const DEMO_STORAGE_KEY = 'nexreach-v25-demo';
...
const MEMBER_NAMES = [
  ...
  'SANTOS, RAFAEL', 'BELL, JASMINE', 'NAKAMURA, YUKI', 'IBRAHIM, HASSAN', 'LANE, PATRICIA',
];
const PLANS = ['Aetna', 'Cigna', 'UnitedHealthcare', 'Humana'];
const ROW_METHODS = ['Onsite', 'Offsite', 'Offsite', 'Onsite', 'EMRR', 'Offsite', 'Onsite', 'Offsite', 'EMRR', 'Offsite', 'Onsite', 'Offsite', 'Offsite', 'Onsite', 'EMRR', 'Offsite', 'Onsite', 'Offsite', 'Onsite', 'Offsite', 'Offsite', 'Onsite', 'Offsite', 'EMRR', 'Onsite'];
...
const REQUEST_ROWS = MEMBER_NAMES.map((member, i) => ({
  ...
  due: ['3/1/2026', '4/1/2026', '5/1/2026'][i % 3],
  commit: i % 3 === 0 ? '3/1/2026' : i % 3 === 1 ? '4/1/2026' : '—',
  status: 'New',
  payment: '—',
  osRef: `87991820${9887 + i}`,
  practitioner: ['BARNES, TAYLOR', 'CHEN, SARAH', 'PATEL, RAJ', 'WILLIAMS, JAMES'][i % 4],
...
function ScheduleModal({ count, onClose, onSubmit }: { count: number; onClose: () => void; onSubmit?: (commitDate?: string, paymentRequired?: boolean) => void }) {
  ...
  // 3-tier cap logic (no PNP24 release section in v2.5)
  const HIGH_CAP = 60;
  const HIGH_CAP_COUNT = 7;
  const LOW_CAP = 30;
  const LOW_CAP_COUNT = count - HIGH_CAP_COUNT;
          ...
          <>
              ...
              <>
                <Box>
                  ...
                  <Box style={{ border: '1px solid #8a8985', borderRadius: 6, padding: '10px 12px' }}>
                    <Text size="xs" fw={600} mb={6} style={{ color: '#4f4e4c' }}>All RRs ({count})</Text>
                    ...
                    {hasBelowCap && (
                      <Flex gap={6} align="flex-start" mt={8}>
                        ...
                        <Box>
                          <Text size="sm" fw={600} style={{ color: '#1f7c53' }}>Payment amount below cap: {belowCapCount}/{count} RRs</Text>
            ...
            {hasOverCap && (
              <Group gap={8} align="flex-start" style={{ cursor: 'pointer' }} onClick={() => setPendAcknowledged(!pendAcknowledged)}>
                ...
                <Text size="sm">I acknowledge that I am pending {pendCount} of the {count} RRs that exceed the payment cap.</Text>
              </Group>
            )}
  ...
  return (
    <ModalOverlay title={`Scheduling ${count} Record Request(s)`} onClose={onClose} size={600}>
      ...
      <Flex gap={0} style={{ borderRadius: 6, overflow: 'hidden', border: '1px solid #006ccf' }}>
        ...
        >Progress Update</Box>
        <Box onClick={() => canProceedToSchedule && !(hasOverCap && !pendAcknowledged) ? setActiveTab('schedule') : undefined}
          style={{ flex: 1, padding: '10px 0', textAlign: 'center', fontSize: 14, fontWeight: 600,
            ...
            cursor: canProceedToSchedule && !(hasOverCap && !pendAcknowledged) ? 'pointer' : 'default', opacity: canProceedToSchedule && !(hasOverCap && !pendAcknowledged) ? 1 : 0.5, borderLeft: '1px solid #006ccf', borderRadius: 6 }}
        >Schedule</Box>
      </Flex>
      ...
      {activeTab === 'progress' ? (
        <>
          ...
          {paymentRequired === 'yes' && (
            <>
              <Box style={{ border: '1px solid #8a8985', borderRadius: 6, padding: '10px 12px' }}>
                <Text size="xs" fw={600} mb={6}>All RRs ({count})</Text>
                <Flex gap="md" align="flex-start">
                  <TextInput
                    placeholder="$ Enter Amount"
                    value={paymentAmount}
                  ...
                  />
                  <Box>
                    <Text size="xs">Payment Cap Per Chart</Text>
                    <Text size="sm" fw={600}>${LOW_CAP}–${HIGH_CAP}</Text>
                  </Box>
                </Flex>
                {hasBelowCap && (
...
function RerouteModal({ count, onClose, onSubmit, siteAccessType, onSiteAccessTypeChange, excludeMethod }: { count: number; onClose: () => void; onSubmit?: () => void; siteAccessType?: string | null; onSiteAccessTypeChange?: (v: string) => void; excludeMethod?: string }) {
  ...
  const [sameAssite, setSameAsSite] = useState(false);
  const [phoneError, setPhoneError] = useState('');
  const [roiSiteId, setRoiSiteId] = useState('');
  const isHIH = method === 'HIH-Major' || method === 'HIH-Other';
  const isOffsite = method === 'Offsite' || method === 'Onsite' || method === 'EMR Remote';
  const isEmbedded = method === 'Embedded';

  const handleSameAsSite = (checked: boolean) => {
    setSameAsSite(checked);
    if (checked) { setSuggestedPhone(SITE_PHONE); setPhoneError(''); }
    else setSuggestedPhone('');
  };

  const validatePhone = (val: string) => {
    const digits = val.replace(/\D/g, '');
    if (val && digits.length < 10) {
      setPhoneError('This phone number is invalid. Enter a valid number or select "Same as phone number in Site Details."');
    } else {
      setPhoneError('');
    }
  };
      ...
      {isOffsite && (
        <Box>
          <TextInput
            ...
            disabled={sameAssite}
            error={phoneError}
            onChange={(e) => { setSuggestedPhone(e.currentTarget.value); setPhoneError(''); }}
            onBlur={(e) => validatePhone(e.currentTarget.value)}
            styles={sameAssite ? { input: { backgroundColor: '#f7f6f4', color: '#6b7280' } } : undefined}
          />
          <Group gap={8} mt={8} style={{ cursor: 'pointer' }} onClick={() => handleSameAsSite(!sameAssite)}>
            <MantineCheckbox size="sm" checked={sameAssite} onChange={(e) => handleSameAsSite(e.currentTarget.checked)} />
            <Text size="sm">Same as phone number in Site Details</Text>
          </Group>
        </Box>
      )}
      {isEmbedded && (
        <TextInput
          label="Suggested ROI Site ID"
          placeholder="Enter ROI Site ID"
          value={roiSiteId}
          onChange={(e) => setRoiSiteId(e.currentTarget.value)}
        />
      )}
      <Textarea label="Notes" required rows={4} value={notes} onChange={(e) => setNotes(e.currentTarget.value)} />
    </ModalOverlay>
...
function EmrrSaveProgressModal({ count, onClose, onSubmit, siteAccessType, onSiteAccessTypeChange, paymentInfo, onPaymentInfoChange }: {
  ...
  const [submissionMethod, setSubmissionMethod] = useState<string | null>(paymentInfo?.submissionMethod || 'fax');
  const [contactEmail, setContactEmail] = useState('mason@manhattanim.com');
    ...
    <ModalOverlay title={`Updating ${count} Record Request(s)`} onClose={onClose} size={600}>
      ...
      {!paymentInfo && (
        <>
          ...
          {paymentRequired === 'yes' && (
            <>
              <Box style={{ border: '1px solid #e7e5df', borderRadius: 8, padding: '12px 16px' }}>
                ...
                {isUnderCap && <Text size="sm" style={{ color: '#059669', marginTop: 8 }}>&#10003; Payment amount below cap</Text>}
                {isOverCap && (
                  <Box style={{ display: 'flex', alignItems: 'flex-start', gap: 6, color: '#a7850d', marginTop: 8 }}>
                    <Text style={{ fontSize: 14 }}>&#9888;</Text>
                    <Box>
          ...
          <Box>
            <Text size="sm" fw={600} mb={4}>Include Provider Package *</Text>
            <Text size="sm" style={{ color: '#4f4e4c', marginBottom: 8 }}>(Provider Packages are always sent for new record requests using the submission method below)</Text>
            <Radio.Group value={includeProviderPkg} onChange={setIncludeProviderPkg}>
              <Group gap="lg"><Radio value="yes" label="Yes" aria-label="Yes" /><Radio value="no" label="No" aria-label="No" /></Group>
            </Radio.Group>
          </Box>
...
function LandingScreen({
  ...
  retrieval, onRetrievalChange, onSearch, onReset,
}: {
  ...
  return (
    <Box style={{ height: '100vh', width: '100vw', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <Box style={{ flex: 1, backgroundColor: '#fff', padding: '20px 20px', overflow: 'auto', width: '100%' }}>
        ...
        <Box style={{ marginTop: 48, paddingTop: 24, borderTop: '1px solid #e7e5df', maxWidth: 480 }}>
          <Text size="xs" style={{ color: '#9ca3af', marginBottom: 8 }}>Demo tools</Text>
          <Button intent="neutral" appearance="outline" size="sm" onClick={onReset}>Reset Demo</Button>
          <Text size="xs" style={{ color: '#9ca3af', marginTop: 6 }}>Clears all saved progress and payment info for this browser.</Text>
        </Box>
      </Box>
    </Box>
  );
...
function WorkspaceScreen({
  ...
  const [ppStatusModalOpen, setPpStatusModalOpen] = useState(false);
  ...
  const [requestRows, setRequestRows] = useState(() => {
    try {
      ...
      return parsed?.requestRows ?? REQUEST_ROWS.map(r => ({ ...r }));
    } catch { return REQUEST_ROWS.map(r => ({ ...r })); }
  });
  ...
  useEffect(() => {
    try {
      ...
      localStorage.setItem(DEMO_STORAGE_KEY, JSON.stringify({ ...current, paymentInfo }));
    } catch {}
  }, [paymentInfo]);
  ...
  const toggleAll = () => {
    const selectable = filteredRows.filter(r => r.status === 'New' && !(isInbound && r.rowMethod === 'EMRR'));
  ...
  const toastMessages: Record<ActionType, string> = isEmrrContext
    ? { schedule: 'Credential Progress Updated', research: 'Record Requests In Research', pend: 'Record Requests Pended', reroute: 'Record Requests Rerouted' }
    : { schedule: 'Record Requests Scheduled', research: 'Record Requests In Research', pend: 'Record Requests Pended', reroute: 'Record Requests Rerouted' };
  ...
  // Scale raw sample counts to total RR count for snapshot display
  const sampleSize = requestRows.length;
  const snapshotCounts: Record<string, number> = sampleSize > 0
    ? Object.fromEntries(Object.entries(statusCounts).map(([k, v]) => [k, Math.round(v / sampleSize * TOTAL_RR_COUNT)]))
    : {};

  const needsActionCount = snapshotCounts['New'] || 0;
  const pastDueCount = 0;
  const unblockedCount = snapshotCounts['In Progress-Unblocked'] || 0;
  ...
  const filteredRows = requestRows.filter(row => {
    ...
    if (searchQuery) {
      ...
      const match = row.id.includes(q) || row.member.toLowerCase().includes(q) || row.plan.toLowerCase().includes(q) || row.practitioner.toLowerCase().includes(q) || row.site.toLowerCase().includes(q) || row.sloc.toLowerCase().includes(q);
    ...
    const dueDateFilter = filters['Project Due Date'];
    if (dueDateFilter && dueDateFilter.size > 0 && !dueDateFilter.has(row.due)) return false;
    const commitFilter = filters['Commitment Date'];
    if (commitFilter && commitFilter.size > 0) {
      const val = commitFilter.has('No Date Set') && row.commit === '—' ? true : commitFilter.has(row.commit);
      if (!val) return false;
    }
    return true;
  });
    ...
    <Box style={{ height: '100vh', width: '100vw', maxWidth: '100vw', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <Box style={{ display: 'flex', flex: 1, overflow: 'hidden', position: 'relative' }}>
        ...
        {siteDetailsCollapsed ? (
          ...
          /* Expanded: full panel with collapse button on gray card right edge */
          <Box style={{ width: 233, minWidth: 233, flexShrink: 0, position: 'relative' }}>
            {/* Collapse button — pinned to right edge of gray card (card has 20px padding) */}
            <Box
            ...
            <Tabs variant="pill" defaultValue="workspace">
              ...
              <Tabs.Panel value="workspace">
                <Stack gap="xl">
                  ...
                  {(() => {
                    ...
                    const gridCols = showPends
                      ...
                      : isOffsiteOrOnsite
                        ...
                        : '1fr 1px auto';
                    return (
                      <Box style={{ border: '1px solid #e7e5df', borderRadius: 8, overflow: 'hidden' }}>
                        ...
                        <Box style={{ backgroundColor: '#eaf5ff', padding: '14px 20px', borderBottom: '1px solid #c3dcf5' }}>
                          <Group gap={0} align="baseline" wrap="nowrap">
                            ...
                            {uniquePlans.length === 1 ? (
                              ...
                              <Tooltip label={uniquePlans.join(', ')} withArrow position="bottom" multiline w={280}>
                        ...
                        <Box style={{ display: 'grid', gridTemplateColumns: gridCols, backgroundColor: '#fff' }}>
                          ...
                          <Box style={{ padding: '12px 16px' }}>
                            ...
                            <Group gap={24}>
                              <Box>
                                ...
                                <Text size="md" fw={700} style={{ color: '#242423' }}>{pastDueCount}</Text>
                              </Box>
                              <Box>
                                <Text size="xs" style={{ color: '#6e6d6a' }}>{isEmrRemote ? 'New' : 'Unscheduled'}</Text>
                                <Text size="md" fw={700} style={{ color: '#242423' }}>{needsActionCount}</Text>
                              </Box>
                              <Box>
                                <Text size="xs" style={{ color: '#6e6d6a' }}>In Progress-Unblocked</Text>
                                <Text size="md" fw={700} style={{ color: '#242423' }}>0</Text>
                              </Box>
                            </Group>
                          </Box>
                          ...
                          {showPends && (
                            <>
                              <Box style={{ padding: '12px 16px' }}>
                                <Text size="xs" fw={600} mb={10} style={{ color: '#6e6d6a', textTransform: 'uppercase', letterSpacing: 0.4 }}>Pends</Text>
                          ...
                          {/* Credential Pipeline — inbound + EMRR; Actioned — offsite/onsite */}
                          {(isInbound || isEmrRemote) ? (
                              ...
                              <Group gap={20}>
                                {[
                                  ...
                                  <Box key={s.key}>
                                    ...
                                    <Text size="md" fw={700} style={{ color: '#242423' }}>{snapshotCounts[s.key] || 0}</Text>
                                  </Box>
                                ))}
                              </Group>
                            </Box>
                            ...
                            <Box style={{ padding: '12px 16px' }}>
                              ...
                              <Group gap={20}>
                                {['Scheduled', 'Progress Logged', ...(retrievalMethod === 'onsite' ? ['No Availability'] : []), 'In Research', 'Rerouted', 'Pended'].map(s => (
                                  <Box key={s}>
                                    <Text size="xs" style={{ color: '#6e6d6a' }}>{s === 'Progress Logged' ? 'Progress Saved' : s}</Text>
                                    <Text size="md" fw={700} style={{ color: '#242423' }}>{snapshotCounts[s] || 0}</Text>
                                  </Box>
                                ))}
                              </Group>
                            </Box>
                        ...
                        {(isInbound || isEmrRemote) && (
                          <Box style={{ borderTop: '1px solid #e7e5df', padding: '12px 16px', backgroundColor: '#fff' }}>
                            ...
                            <Group gap={20}>
                              {(isInbound
                                ? ['Scheduled', 'Progress Logged', 'No Availability', 'In Research', 'Rerouted', 'Pended']
                  ...
                  <Box>
                      ...
                      <Flex justify="space-between" align="flex-start" gap={8} mb={(retrievalMethod === 'onsite' || isInbound) ? 8 : 0}>
                        <Box style={{ width: 300, flexShrink: 0 }}>
                          <Box style={{ display: 'flex', alignItems: 'center', gap: 4, border: '1px solid #8a8985', borderRadius: 6, padding: '6px 12px', paddingRight: 8, backgroundColor: '#fff' }}>
                            <input placeholder="Search by Member Name, Practitioner, Site, or Request ID..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} style={{ border: 'none', outline: 'none', fontSize: 14, width: '100%', color: '#4f4e4c', background: 'transparent', fontFamily: 'DM Sans, sans-serif' }} />
                            <IconSearch size={16} color="#6e6d6a" style={{ flexShrink: 0 }} />
                          </Box>
                        </Box>
                        <Group gap={8} wrap="wrap" justify="flex-end" style={{ flex: 1 }}>
                          <FilterPill label="Project Due Date" options={['3/1/2026', '4/1/2026', '5/1/2026']} selected={filters['Project Due Date']} onToggle={(v) => toggleFilter('Project Due Date', v)} />
                          <FilterPill label="Commitment Date" options={['3/1/2026', '4/1/2026', 'No Date Set']} selected={filters['Commitment Date']} onToggle={(v) => toggleFilter('Commitment Date', v)} />
                    ...
                    {selectedRows.size > 0 && canTakeAction && (
                      <Box style={{ backgroundColor: '#f3f0ff', border: '1px solid #7c3aed', borderRadius: 6, padding: '8px 16px', marginBottom: 12, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <Group gap={8} align="center">
                          ...
                          <Text size="sm" fw={500} style={{ color: '#242423' }}>{selectedRows.size}/{TOTAL_RR_COUNT.toLocaleString()} Requests Selected</Text>
                        ...
                        <Table highlightOnHover style={{ minWidth: 1500, borderCollapse: 'collapse' }}>
                          <Table.Thead>
                            <Table.Tr style={{ backgroundColor: '#f7f6f4', borderBottom: '1px solid #e7e5df' }}>
                              <Table.Th style={{ width: 40, padding: '8px' }}><MantineCheckbox size="xs" disabled={!canTakeAction} checked={selectedRows.size > 0 && selectedRows.size === filteredRows.filter(r => r.status === 'New' && !(isInbound && r.rowMethod === 'EMRR')).length} indeterminate={selectedRows.size > 0 && selectedRows.size < filteredRows.filter(r => r.status === 'New' && !(isInbound && r.rowMethod === 'EMRR')).length} onChange={toggleAll} /></Table.Th>
                          ...
                          <Table.Tbody style={{ opacity: canTakeAction ? 1 : 0.5, pointerEvents: canTakeAction ? 'auto' : 'none' }}>
                            {filteredRows.map((row) => {
                              const isActioned = row.status !== 'New';
                    ...
                    {/* Pagination */}
                    <Group justify="space-between" align="center" mt="md" style={{ borderTop: '1px solid #e7e5df', paddingTop: 8, background: '#f7f6f4' }}>
                      <Group gap={8} px={12}>
                        <Text size="sm" c="gray.5">Items per page:</Text>
                        <Select size="xs" value="25" data={['10','25','50','100']} w={70} aria-label="Items per page" />
                        <Box style={{ width: 1, height: 20, background: '#e7e5df' }} />
                        <Text size="sm" c="gray.5">1–{filteredRows.length} of {TOTAL_RR_COUNT.toLocaleString()} items</Text>
                      </Group>
                      <Group gap={8} px={12}>
                        <Text size="sm" c="gray.5">Page:</Text>
                        <Select size="xs" value="1" data={Array.from({length: Math.ceil(TOTAL_RR_COUNT / 25)}, (_, i) => String(i + 1))} w={60} aria-label="Page" />
                        <Text size="sm" c="gray.5">of {Math.ceil(TOTAL_RR_COUNT / 25)}</Text>
                        <Group gap={4}>
                          <ActionIcon intent="neutral" appearance="outline" size="sm" aria-label="First page"><FirstIcon /></ActionIcon>
                          <ActionIcon intent="neutral" appearance="outline" size="sm" aria-label="Previous page"><ChevronLeftIcon /></ActionIcon>
                          <ActionIcon intent="neutral" appearance="outline" size="sm" aria-label="Next page"><DartChevronRight /></ActionIcon>
                          <ActionIcon intent="neutral" appearance="outline" size="sm" aria-label="Last page"><LastIcon /></ActionIcon>
                        </Group>
                      </Group>
                    </Group>
                  </Box>
              ...
              <Tabs.Panel value="history">
                <Stack gap="md">
                  ...
                  {/* Pagination */}
                  <Group justify="space-between" align="center" style={{ borderTop: '1px solid #e7e5df', paddingTop: 8, background: '#f7f6f4' }}>
                    <Group gap={8} px={12}>
                      <Text size="sm" c="gray.5">Items per page:</Text>
                      <Select size="xs" value="25" data={['10','25','50','100']} w={70} aria-label="Items per page" />
                      <Box style={{ width: 1, height: 20, background: '#e7e5df' }} />
                      <Text size="sm" c="gray.5">1–2 of 2 items</Text>
                    </Group>
                    <Group gap={8} px={12}>
                      <Text size="sm" c="gray.5">Page:</Text>
                      <Select size="xs" value="1" data={['1']} w={60} aria-label="Page" />
                      <Text size="sm" c="gray.5">of 1</Text>
                      <Group gap={4}>
                        <ActionIcon intent="neutral" appearance="outline" size="sm" aria-label="First page"><FirstIcon /></ActionIcon>
                        <ActionIcon intent="neutral" appearance="outline" size="sm" aria-label="Previous page"><ChevronLeftIcon /></ActionIcon>
                        <ActionIcon intent="neutral" appearance="outline" size="sm" aria-label="Next page"><DartChevronRight /></ActionIcon>
      ...
      {ppStatusModalOpen && (
        <>
          <div onClick={() => setPpStatusModalOpen(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', zIndex: 9998 }} />
          <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', background: '#fff', borderRadius: 12, zIndex: 9999, width: 780, maxHeight: '80vh', overflowY: 'auto', padding: '24px', boxShadow: '0 8px 32px rgba(0,0,0,0.2)' }}>
            <Group justify="space-between" align="center" mb={20}>
              <Text fw={600} size="lg">Provider Package Status</Text>
              <Text style={{ cursor: 'pointer', color: '#6b7280', fontSize: 20, lineHeight: 1 }} onClick={() => setPpStatusModalOpen(false)}>×</Text>
            </Group>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead>
                <tr style={{ borderBottom: '1px solid #e7e5df', backgroundColor: '#f7f6f4' }}>
                  {['Status', 'Failure Reason', 'Assigned Fax Number', 'PP ID', 'Timestamp'].map(h => (
                    <th key={h} style={{ padding: '8px 12px', textAlign: 'left', fontWeight: 500, color: '#4f4e4c' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
...
const V25_METHOD_SLUGS: Record<string, { method: string; callType: string }> = {
...
export function NexReachV25() {
  ...
  // Parse path: /nexreach-v2-5/[methodSlug]
  const pathParts = location.pathname.replace(/\/+$/, '').split('/').filter(Boolean);
  // pathParts[0] === 'nexreach-v2-5'
  const methodSlug = pathParts[1];
  const isValidMethod = methodSlug && methodSlug in V25_METHOD_SLUGS;

  if (isValidMethod) {
    const { method, callType: ct } = V25_METHOD_SLUGS[methodSlug];
    return (
      <WorkspaceScreen
        onBackToSearch={() => navigate('/nexreach-v2-5')}
